...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER_VECTOR(rss_coef_link);
  vector<Type> rss_coef(int(2));
  rss_coef[0] = rss_coef_link[0];
  rss_coef[1] = exp(rss_coef_link[1]);
  ...
  for (int i=0; i<n_masks; i++){
    for (int j=0; j<n_traps; j++){
      ...
      mu = rss_coef[1]* mask_ss(i,j) + rss_coef[0];
      sigma = rss_coef[1]* sigma_ss;
      mask_probs(i,j) = invlogit(mu/ sqrt(Type(1) + Type(0.368) * pow(sigma, 2)));
  ...
  for (int i=0; i<n_masks; i++){
    for (int j=0; j<n_traps; j++){
      // get signal strength for corresponding distance
      if (mask_dists(i,j) > Type(1)){
        noise_mask_ss(i,j) = noise_b0 -log10(mask_dists(i,j))* Type(20) - noise_b1*(mask_dists(i,j) - Type(1));
      }
      else {
        noise_mask_ss(i,j) = noise_b0;
      }
      // get detection probs for corresponding signal strength
      mu = rss_coef[1]* noise_mask_ss(i,j) + rss_coef[0];
      sigma = rss_coef[1]* noise_sigma_ss;
      noise_mask_probs(i,j) = invlogit(mu/ sqrt(Type(1) + Type(0.368) * pow(sigma, 2)));
  ...
  for (int i=0; i<n_obs; i++){
    ...
    num_det = Type(0);
    conf_sum = Type(0.0);
    // get num of detection and sum of confidence
    ...
    for (int m = 0; m < 2; m++){
      // get likelihood for confidence mean
      if (m == int(1)){
      ...
      for (int j=0; j<n_masks; j++){
        ...
        if (m == int(1)){
          for (int k=0; k<n_traps; k++){
            ...
            if (capt_bin(i,k) == Type(1)){
              log_f_sig += (-log(Type(1) + exp(-rss_coef[1]*capt_ss(i,k) - rss_coef[0]) + DBL_MIN)
                                   + dnorm(capt_ss(i,k), mask_ss(j,k), sigma_ss, int(1)));
            } else {
              // binary capture history likelihood
          ...
          for (int k=0; k<n_traps; k++){
            ...
            if (capt_bin(i,k) == Type(1)){
              log_f_sig += (-log(Type(1) + exp(-rss_coef[1]*capt_ss(i,k) - rss_coef[0]) + DBL_MIN)
                                      + dnorm(capt_ss(i,k), noise_mask_ss(j,k), noise_sigma_ss, int(1)));
            } else{
              // binary capture history likelihood
